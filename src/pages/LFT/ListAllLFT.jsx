...
const ListAllLFT = () => {
  const { listAllLFT, deleteReport, getReport } = useFirebase();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setIsLoading(true);
      try {
        const fetchedReports = await listAllLFT();
  ...
  const handleViewReport = async (reportId) => {
    ...
    if (reportData) {
      navigate(`/admin/listAllReports/view/${reportId}`, { state: reportData });
    } else {
          ...
          <ul>
            ...
            {filteredReports
              ...
              .map((report, index) => (
                ...
                >
                  <p className="w-[40%] text-gray-600 font-medium ">
                    {report.name}
                  </p>
                  <p className="w-[25%]  text-gray-600 font-medium ">
                    {report.date}
                  </p>
                  <button
                    className="w-[25%]"
