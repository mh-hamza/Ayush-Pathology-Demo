...
const ListAllRFTComponent = () => {
  const { listAllRFTReport, deleteRFTReport, viewRFTReport } = useFirebase();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setIsLoading(true);
      try {
        const fetchedReports = await listAllRFTReport();
  ...
  const handleDeleteReport = async (reportId) => {
    if (window.confirm("Are you sure you want to delete this report?")) {
      try {
        await deleteRFTReport(reportId);
  ...
  const handleViewReport = async (reportId) => {
    const reportData = await viewRFTReport(reportId);
    if (reportData) {
      navigate(`/admin/listAllRFT/view/${reportId}`, { state: reportData });
    } else {
          ...
          <ul>
            ...
            {filteredReports
              ...
              .map((report, index) => (
                ...
                >
                  <p className="w-[40%] text-gray-800 font-semibold">{report.name}</p>
                  <p className="w-[25%] text-gray-800 font-semibold">{report.date}</p>
                  <button
                    className="w-[25%]"
...
export default ListAllRFTComponent;
