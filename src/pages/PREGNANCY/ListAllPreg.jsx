...
const ListAllPreg = () => {
  const { listAllPregReport, deletePregReport, viewPregReport } = useFirebase();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setIsLoading(true);
      try {
        const fetchedReports = await listAllPregReport();
  ...
  const handleDeleteReport = async (reportId) => {
    if (window.confirm("Are you sure you want to delete this report?")) {
      try {
        await deletePregReport(reportId);
  ...
  const handleViewReport = async (reportId) => {
    const reportData = await viewPregReport(reportId);
    if (reportData) {
      navigate(`/admin/listAllPregReport/view/${reportId}`, { state: reportData });
    } else {
