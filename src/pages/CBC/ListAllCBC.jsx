...
const ListAllCBCcomponents = () => {
  const { listAllCBCS, deleteReportCBC, viewReportCBC } = useFirebase();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setIsLoading(true);
      try {
        const fetchedReports = await listAllCBCS();
  ...
  const handleDeleteReport = async (reportId) => {
    if (window.confirm("Are you sure you want to delete this report?")) {
      try {
        await deleteReportCBC(reportId);
  ...
  const handleViewReport = async (reportId) => {
    const reportData = await viewReportCBC(reportId);
    if (reportData) {
      navigate(`/admin/listAllCBC/view/${reportId}`, { state: reportData });
    } else {
...
export default ListAllCBCcomponents;
