...
const ListAllWidal = () => {
  const { listAllWidalReport, deleteWidalReport, viewWidalReport } = useFirebase();
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setIsLoading(true);
      try {
        const fetchedReports = await listAllWidalReport();
  ...
  const handleDeleteReport = async (reportId) => {
    if (window.confirm("Are you sure you want to delete this report?")) {
      try {
        await deleteWidalReport(reportId);
  ...
  const handleViewReport = async (reportId) => {
    const reportData = await viewWidalReport(reportId);
    if (reportData) {
      navigate(`/admin/listAllWidal/view/${reportId}`, { state: reportData });
    } else {
